...
// "Bitcoin seed" is the HMAC key for the master node. It doubles as a domain
// separator so a seed cannot be confused with input to any other keyed
// derivation.
static const char kMasterKey[] = "Bitcoin seed";

// Wipe sensitive bytes; mirrors WalletEnvelopeManager's secureClearData for the
// raw buffers used here.
static void secureClear(void *ptr, size_t len) {
    if (!ptr || len == 0) return;
    (void)memset_s(ptr, len, 0, len);
}

// Serialize a uint32 as 4 big-endian bytes (ser32).
...
bool bip32MasterKey(const secp256k1_context *ctx,
    ...
    // Left 32 bytes -> master private key; right 32 -> chain code.
...
bool bip32CKDPriv(const secp256k1_context *ctx,
    ...
    // The HMAC input is 37 bytes in both cases: a 33-byte left part followed by
    // ser32(index).
    uint8_t data[37];
    if (index >= BIP32_HARDENED) {
        // Hardened: 0x00 || parent priv || ser32(index).
        data[0] = 0x00;
        memcpy(data + 1, parent->priv, 32);
    } else {
        // Non-hardened: ser_compressed(parent pub) || ser32(index).
    ...
    if (ok) {
        ...
        memcpy(out->chainCode, I + 32, 32);  // child chain code = IR
    }
...
bool bip32Derive(const secp256k1_context *ctx,
    ...
    const char *p = path;
    if (*p != 'm' && *p != 'M') return false;  // path is rooted at the master node
    p++;

    bool ok = true;
    while (ok && *p) {
        if (*p != '/') { ok = false; break; }  // segments are slash-separated
        p++;
        ...
        while (*p >= '0' && *p <= '9') {
            ...
            if (index >= BIP32_HARDENED) { ok = false; break; }  // index out of range
            p++;
        }
        if (!ok) break;
